...
myApp.controller('myCtrl', function($scope, $filter) {
	...
	$scope.schedules = [{
		...
		id: 2,
		jobName: ['AC Acute Chief', 'AC Acute Consult', 'AC Acute Floor', 'AC Blue Floor', 'AC Blue Consult', 'AC ES Floor', 'AC ES Assist', 'AC ES Chief', 'AC Trauma Floor', 'AC Trauma Chief', 'AC ES2 Chief', 'AC SICU', 'AC Call Chief', 'AC Call Consult', 'AC Call Floor', 'AC Call SICU', 'AC Call Trauma'],
	...
	$scope.saveScheduleEdit = function(scheduleId) {
		if (scheduleId == 'new') {
			var newSchedule = {
				jobName: $scope.scheduleFormJobName,
			...
			$scope.schedules[scheduleId].jobName = $scope.scheduleFormJobName;
	...
	$scope.findSchedule = function(schedule) {
		...
		for (let j = 0, job; j < $scope.jobs.length; j++) {
			...
			for (let n = 0; n < $scope.schedules[index].jobName.length; n++) {
				if ($scope.jobs[j].name == $scope.schedules[index].jobName[n]) {
					job = $scope.jobs[j];
					jobs[job.id] = job;
					job._id = job.id
	...
	$scope.today = function() {
		$scope.viewDate = moment().add('1',"day").format("ddd, DD MMM YYYY");
		$scope.count = 0;
	}
	$scope.increment = function() {
		$scope.viewDate = moment().add($scope.count++, "day").format("ddd, DD MMM YYYY");
	}
